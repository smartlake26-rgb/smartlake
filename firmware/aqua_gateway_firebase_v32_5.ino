// ================================================================
//  AQUA GATEWAY (LoRa <-> WiFi/FIREBASE) v32.4  -  ESP32 + SX1278/Ra-02
//  Baliq ko'llari tizimi SHLYUZI  —  FIREBASE versiyasi
//
//  v32.5 O'ZGARISHLAR (buyruqdan keyin ilova avtomatik yangilanishi):
//    Fix S1 — AVTO HOLAT SO'ROVI: node biror buyruqni (MINDO/FARQ/KRITIK/
//             AER/MODE) CMDACK bilan tasdiqlashi bilan gateway ~0.6s dan
//             keyin O'ZI CMD_STATUS yuboradi -> node darhol yangi telemetriya
//             jo'natadi -> ilovadagi qiymatlar "Holat so'rash" bosilmasdan
//             bir necha soniyada yangilanadi.
//    Fix S2 — BIR POLL = BIR BUYRUQ: /commands da bir nechta buyruq turgan
//             bo'lsa, avval faqat bittasi yuboriladi, qolganlari keyingi
//             poll'da (2s). Sabab: LoRa yarim dupleks — node birinchi
//             buyruqqa CMDACK uzatayotganda ketma-ket kelgan ikkinchi
//             buyruqni eshitmay yo'qotardi.
//
//  v32.4 O'ZGARISHLAR (TOO_MANY_ATTEMPTS_TRY_LATER muammosi):
//    Fix T1 — FB_KOD_CREDS_MAJBUR: sinov davrida KOD ICHIDAGI Firebase
//             email/parol NVS'dagini BOSIB YOZADI. Oldin portal orqali
//             xato parol saqlangan bo'lsa, qurilma o'sha xato parol bilan
//             qayta-qayta urinib, Firebase akkauntni vaqtincha bloklardi
//             (TOO_MANY_ATTEMPTS_TRY_LATER). Endi doim kod paroli ishlatiladi.
//             ISHLAB CHIQARISHGA O'TISHDA: 0 ga qaytaring (portal rejimi).
//    Fix T2 — NVS'dan bo'sh qiymat kod defaultini o'chirib yubormaydi
//             (faqat NVS'da haqiqiy qiymat bo'lsa yuklanadi).
//    Fix T3 — Token xatosida qayta urinishlar SONI CHEKLANMAGAN (sinov davri),
//             lekin urinishlar orasi 30s — bu cheklov emas, cheksiz davom etadi;
//             shunchaki Firebase'ni "bombardimon" qilib blokni yangilamaslik uchun.
//    Fix T4 — Boot'da qaysi email/API key ishlatilayotgani (maskalab) chop etiladi —
//             NVS/kod chalkashligini darhol ko'rish mumkin.
//
...
#include <WiFi.h>
#include <esp_wifi.h>   // v32.3 DIAG: saqlangan SSID va reason kodlarini o'qish uchun
...
//  v32.4 Fix T1: FB_KOD_CREDS_MAJBUR=1 bo'lsa (SINOV) — quyidagi qiymatlar
//  DOIM ishlatiladi va NVS ustidan yoziladi. =0 bo'lsa (ISHLAB CHIQARISH) —
//  NVS/portal ustuvor (Fix B rejimi).
//  Portalni ochish: 3x tez yoqib-o'chirish yoki BOOT tugmasini bosib yoqish.
// ================================================================
static char fbApiKey[80]    = "AIzaSyA2JgIbG_kJCLSEjsU142kWyMqTBrUlTdE";   // Firebase API Key  (AIza... bilan boshlanadi)
static char fbDbUrl[128]    = "https://smartlake-6ce23-default-rtdb.europe-west1.firebasedatabase.app";   // Realtime DB URL   (https://...firebasedatabase.app)
static char fbEmail[64]     = "smartlake26@gmail.com";   // Device user email (device@yourdomain.uz)
static char fbPassword[64]  = "Sm@rtL@ke26!";   // Device user paroli

// ================================================================
//  KOMPILYATSIYA SOZLAMALARI
// ================================================================
// Fix T1: 1 = SINOV REJIMI — yuqoridagi kod credentials DOIM ishlatiladi,
//         NVS'dagi eski/xato qiymatlar bosib yoziladi (portal FB maydonlari
//         faqat ko'rsatish uchun qoladi, WiFi sozlash ishlayveradi).
//         0 = ISHLAB CHIQARISH — eski xatti-harakat: NVS/portal ustuvor.
#define FB_KOD_CREDS_MAJBUR  1

#define USE_LCD            1     // 0 -> LCD'siz (headless) ishlaydi

// WiFi tiklanish siyosati
#define WIFI_KICK_S        20    // v32.3: 45 -> 20 s (tezroq qayta urinish)
...
// v32.5 Fix S1: buyruq tasdiqlangach avtomatik CMD_STATUS rejalashtirish
static uint32_t      autoStatusId = 0;   // 0 = rejalashtirilmagan
static unsigned long autoStatusAt = 0;   // qachon yuborilsin (millis)
...
// v32.5 Fix S1: rejalashtirilgan avtomatik CMD_STATUS ni yuborish (non-blocking).
// loop() dan har aylanishda chaqiriladi; vaqti kelganda bir marta yuboradi.
void autoStatusYubor() {
  if (!autoStatusId) return;
  if ((long)(millis() - autoStatusAt) < 0) return;   // hali erta (overflow-safe)
  Serial.printf("[CMD] Avto holat so'rovi -> %08X\n", (unsigned)autoStatusId);
  loraCmdYubor(autoStatusId, CMD_STATUS, 0);
  autoStatusId = 0;
}
...
void loraQabulTekshir() {
  ...
  if (type == FT_TELE) {
    ...
    if (WiFi.status() == WL_CONNECTED && fbBoshlandi && Firebase.ready()) {  // v32.2 FIX
    ...
    // v32.5 Fix S1: STATUS/HIST bo'lmagan buyruq muvaffaqiyatli tasdiqlandi ->
    // biroz kutib (node radiosi RX ga qaytsin) avtomatik holat so'raymiz,
    // shunda ilova yangi qiymatni darhol ko'radi.
    if (ok == 1 && cmd != CMD_STATUS && cmd != CMD_HIST) {
      autoStatusId = src;
      autoStatusAt = millis() + 600;
    }
...
void buyruqlarniTekshir() {
  ...
  for (JsonPair kv : cmdDoc.as<JsonObject>()) {
    ...
    if (id) {
      ...
      if (ok) {
        loraCmdYubor(id, cmd, val);
        // v32.5 Fix S2: bir poll'da faqat BITTA buyruq yuboriladi.
        // Node CMDACK uzatayotganda ketma-ket ikkinchi buyruq efirda
        // to'qnashib yo'qolardi. Qolgan buyruqlar RTDB'da qoladi va
        // keyingi poll'da (2s dan keyin) yuboriladi.
        Firebase.RTDB.deleteNode(&fbdo, dpath.c_str());
        esp_task_wdt_reset();
        return;
      }
...
void firebaseBoshla() {
  ...
  // v32.1: SSL/server javob kutish chegaralari — bloklanish qisqaradi
  fbconfig.timeout.serverResponse    = 10 * 1000;  // 10 s
  fbconfig.timeout.socketConnection  = 10 * 1000;  // 10 s

  // Fix T3 (SINOV): urinishlar soni CHEKLANMAGAN — max_token_generation_retry
  // ATAYIN o'rnatilmaydi, kutubxona token olguncha urinaveradi.
  // Lekin xato bo'lsa urinishlar ORASI 30s qilinadi (default ~5s edi).
  // Bu cheklov EMAS: har 5s da urinish Firebase'ning TOO_MANY_ATTEMPTS
  // blokini qayta-qayta yangilab, hech qachon ochilmasligiga sabab bo'lardi.
  fbconfig.timeout.tokenGenerationError = 30 * 1000;  // 30 s

  Serial.printf("[FB] login urinilmoqda: %s\n", fbEmail);

  // FIX #4: NTP vaqt sinxronizatsiyasi (eski buyruqlarni aniqlash uchun)
  configTime(0, 0, "pool.ntp.org", "time.nist.gov");

  // v32.2 FIX (ILDIZ SABAB): reconnectWiFi(true) — mobizt kutubxonasi WiFi uzilganini
  // ko'rsa O'ZI ichidan WiFi.reconnect() (disconnect+begin) chaqiradi. Bu bizning
  // wifiNazorat() va setAutoReconnect(true) bilan RAQOBATLASHIB, davom etayotgan
  // assotsiatsiya/DHCP jarayonini har safar qaytadan boshlatadi -> WiFi hech qachon
  // ulanib ulgurmaydi. WiFi ni FAQAT wifiNazorat() boshqaradi.
  Firebase.reconnectWiFi(false);
  fbdo.setBSSLBufferSize(4096, 1024);
  fbdo.setResponseSize(4096);
  // Fix P: SSL/token handshake WDT dan vaqtincha chiqariladi (bloklanish xavfsiz)
...
//  ============   WiFi + SOZLAMALAR (NVS)   ============
// ================================================================
// Fix T2 yordamchi: NVS'dan faqat BO'SH BO'LMAGAN qiymatni yuklaydi.
// (Eski kod bo'sh NVS kaliti bilan kod defaultini o'chirib yuborishi mumkin edi.)
#if !FB_KOD_CREDS_MAJBUR
static void nvsYuklaAgarBor(const char* kalit, char* buf, size_t buflen) {
  char tmp[128];
  size_t n = prefs.getString(kalit, tmp, sizeof(tmp));
  if (n > 0 && tmp[0] != '\0') strlcpy(buf, tmp, buflen);
}
#endif

void sozlamalarniYukla() {
#if FB_KOD_CREDS_MAJBUR
  // Fix T1 (SINOV): Kod ichidagi Firebase credentials MAJBURAN ishlatiladi.
  // NVS'dagi eski/xato qiymatlar kod qiymatlari bilan yangilab qo'yiladi —
  // shunda keyinchalik FB_KOD_CREDS_MAJBUR=0 qilinsa ham to'g'ri qiymat qoladi.
  prefs.putString("fbApiKey",   fbApiKey);
  prefs.putString("fbDbUrl",    fbDbUrl);
  prefs.putString("fbEmail",    fbEmail);
  prefs.putString("fbPassword", fbPassword);
  Serial.println("[CFG] Fix T1: SINOV rejimi — KOD ichidagi Firebase credentials ishlatiladi (NVS yangilandi)");
#else
  // Fix B + Fix T2: Firebase credentials NVS'dan yuklanadi,
  // lekin faqat NVS'da haqiqiy (bo'sh bo'lmagan) qiymat bo'lsa.
  nvsYuklaAgarBor("fbApiKey",   fbApiKey,   sizeof(fbApiKey));
  nvsYuklaAgarBor("fbDbUrl",    fbDbUrl,    sizeof(fbDbUrl));
  nvsYuklaAgarBor("fbEmail",    fbEmail,    sizeof(fbEmail));
  nvsYuklaAgarBor("fbPassword", fbPassword, sizeof(fbPassword));
#endif
  ...
  // Fix T4: qaysi credentials ishlatilayotganini maskalab ko'rsatish.
  // Parol to'liq chiqarilmaydi — faqat uzunligi va oxirgi belgisi.
  size_t plen = strlen(fbPassword);
  Serial.printf("[CFG] GATEWAY_ID=0x%08X | FB API: %s\n",
                (unsigned)GATEWAY_ID, strlen(fbApiKey) > 0 ? "✓" : "bo'sh");
  Serial.printf("[CFG][diag] FB email: %s | parol: %u belgi (oxiri '%c') | API key oxiri: ...%s\n",
                strlen(fbEmail) ? fbEmail : "(bo'sh)",
                (unsigned)plen, plen ? fbPassword[plen-1] : '?',
                strlen(fbApiKey) > 6 ? fbApiKey + strlen(fbApiKey) - 6 : fbApiKey);
}

// Sozlash portali (WiFi + Firebase credentials + Gateway ID).
bool sozlashPortali(bool cheksiz) {
  WiFiManager wm;
  wm.setConnectTimeout(30);        // v32: 20s -> 30s (sekin router/DHCP uchun)
  wm.setConnectRetries(3);         // v32: ESP32'da 1-urinish ba'zan bekorga yiqiladi
  wm.setCleanConnect(true);        // v32: ulanishdan oldin eski holatni tozalash (ESP32 fix)
...
void ishchiWifiBoshla() {
  ...
  // v32.3 DIAG: NVS'da SSID umuman bormi — tekshiramiz.
  // Bo'sh bo'lsa cheksiz urinishning ma'nosi yo'q: darhol portal ochiladi.
  wifi_config_t wconf;
  if (esp_wifi_get_config(WIFI_IF_STA, &wconf) == ESP_OK) {
    Serial.printf("[WiFi][diag] saqlangan SSID: '%s'\n", (const char*)wconf.sta.ssid);
    if (wconf.sta.ssid[0] == '\0') {
      Serial.println("[WiFi] XATO: saqlangan SSID BO'SH — sozlash portali ochilyapti!");
      sozlashPortali(true);
      return;
    }
  }
...
void wifiNazorat() {
  ...
  if (taymerOtdimi(lastWifiKick, (unsigned long)WIFI_KICK_S * 1000UL)) {
    lastWifiKick = millis();
    Serial.printf("[WiFi] qayta ulanishga urinish... (status=%d, %lus uzilgan)\n",
                  (int)WiFi.status(), (unsigned long)((millis() - wifiUzilganVaqt) / 1000UL));
    WiFi.disconnect();
    delay(100);   // v32.2 FIX: ESP32'da disconnect ASINXRON — kechikkan disconnect
                  // hodisasi darhol chaqirilgan begin() ulanishini bekor qilardi
    WiFi.begin();
  }
...
void lcdHolat() {
  ...
  bool f = (w && fbBoshlandi && Firebase.ready());  // v32.2 FIX: WiFi guard (har 1s chaqiriladi!)
...
// ================================================================
//  OTA: GATEWAY O'ZINI YANGILASH (ota_gateway)
// ================================================================
// ================================================================
...
// ================================================================
//  OTA RELAY: NODE'GA LORA ORQALI FIRMWARE UZATISH (ota_relay)
// ================================================================
// ================================================================
...
void setup() {
  ...
  // v32 FIX: WDT avval ro'yxatga olinadi (loraBoshlash ichida wdt_reset chaqiriladi,
  //          aks holda "task_wdt: task not found" xatosi chiqadi)
  // v32.1 FIX: 10s -> 30s. Firebase SSL token handshake zaif WiFi'da 10s dan
  //            uzoq bloklashi mumkin — 10s WDT qurilmani restart siklga tushirardi.
  ...
  // v32.3 DIAG: har bir uzilishning ANIQ SABABI (reason kodi) chop etiladi.
  // 201=AP topilmadi | 202/15/204=parol xato | 2=auth muddati | 8=biz uzdik
#if defined(ESP_ARDUINO_VERSION_MAJOR) && ESP_ARDUINO_VERSION_MAJOR >= 2
  WiFi.onEvent([](WiFiEvent_t e, WiFiEventInfo_t info){
    Serial.printf("[WiFi][diag] UZILDI reason=%d\n", (int)info.wifi_sta_disconnected.reason);
  }, ARDUINO_EVENT_WIFI_STA_DISCONNECTED);
  WiFi.onEvent([](WiFiEvent_t e, WiFiEventInfo_t info){
    Serial.printf("[WiFi][diag] ULANDI IP=%s RSSI=%d\n",
  ...
  WiFi.onEvent([](WiFiEvent_t e, WiFiEventInfo_t info){
    Serial.printf("[WiFi][diag] ULANDI IP=%s\n", WiFi.localIP().toString().c_str());
  ...
  // v32 FIX: Firebase bo'sh bo'lsa ham portal MAJBURAN qayta ochilmaydi.
  //          WiFi saqlangan bo'lsa qurilma normal ishga tushadi;
  //          Firebase keyinroq 3x o'chirib-yoqish (portal) orqali kiritiladi.
  bool portalKerak = majburiyPortal || !sozlangan;
  if (portalKerak) {
    sozlashPortali(!sozlangan);   // WiFi yo'q bo'lsa cheksiz, aks holda 5 daq
  } else {
    if (!fbTayyor) Serial.println("[FB] DIQQAT: Firebase sozlanmagan — 3x o'chirib-yoqib portalda kiriting");
    ishchiWifiBoshla();
    // v32.2 FIX: Firebase FAQAT WiFi ulangandan keyin ishga tushiriladi.
    // Avval WiFi ulanmagan holda ham chaqirilardi — kutubxona ulanish
    // jarayoniga aralashib uni buzardi. Ulanmasa loop() da keyinroq boshlaydi.
    if (WiFi.status() == WL_CONNECTED) firebaseBoshla();
  }
...
void loop() {
  ...
  // 1.5) v32.5 Fix S1: tasdiqlangan buyruqdan keyin avto holat so'rovi
  autoStatusYubor();

  // 2) WiFi nazorati (uzilsa tiklash / avariya portali)
  wifiNazorat();

  // 2.5) v32.2 FIX: setup'da WiFi ulgurmagan bo'lsa — Firebase shu yerda boshlaydi
  if (WiFi.status() == WL_CONNECTED && !fbBoshlandi) {
    static bool fbUrinildi = false;
    if (!fbUrinildi) { fbUrinildi = true; firebaseBoshla(); }
  }
...
void histKaydYoz(uint32_t src, uint8_t idx, uint8_t total,
                  uint8_t oy, uint8_t kun, uint8_t soat,
                  uint8_t daqiqa, uint8_t doB) {
  if (WiFi.status() != WL_CONNECTED || !fbBoshlandi || !Firebase.ready()) return;  // v32.2 FIX
  char nom[16]; idGaNom(src, nom);
