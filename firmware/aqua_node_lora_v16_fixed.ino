// ================================================================
//  AQUA NODE (LoRa) v16.0 - ESP32 DevKit USB-C + SX1278/Ra-02 433MHz
...
//      Ra-02 RST   -> GPIO 25   // BUG-H2 FIX: GPIO 2 dan GPIO 25 ga ko'chirildi; Row5 endi GPIO 2
...
//    KEYPAD ROW : GPIO 12,14,27,26,2  | COL : GPIO 33,32,13,19  // BUG-H1 FIX: Row5: 25->2
...
byte rowPins[ROWS] = {12, 14, 27, 26, 2};  // BUG-H1 FIX: Row5 GPIO25->2 (LORA_RST=25 konflikt hal)
...
void loraBuyruqQolla(uint8_t cmd, int32_t val) {
  switch (cmd) {
    ...
    case CMD_AER_AUTO:
      ...
      break;
    case CMD_MINDO:
      if (val >= 1 && val <= 20) { minDo_x10 = (int16_t)(val*10); prefs.putShort("minDo", minDo_x10); }
      break;
    case CMD_FARQ:
      if (val >= 1 && val <= 10) { farqDo_x10 = (int16_t)(val*10); prefs.putShort("farqDo", farqDo_x10); }
      break;
    case CMD_KRITIK:
      if (val >= 1 && val <= 20) { kritikDo_x10 = (int16_t)(val*10); prefs.putShort("kritikDo", kritikDo_x10); alarmFaol = false; }
      break;
    case CMD_MODE:
